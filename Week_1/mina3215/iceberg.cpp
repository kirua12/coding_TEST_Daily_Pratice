...
int main()
{
    ...
    while(1)
    {
        ...
        // BFS로 연결된 빙산 덩어리 개수 세기 (labeling 대체)
        int label_count = 0;
            ...
            for(int j = 0; j < col; j++)
            {
                if(iceberg[i][j] != 0 && !visited[i][j])
                {
                    ...
                    while(head < tail)
                    {
                        ...
                        if(x-1 >= 0 && iceberg[x-1][y] != 0 && !visited[x-1][y]) //상
                        {
                            visited[x-1][y] = true;
                            qx[tail] = x-1; qy[tail] = y; tail++;
                        }
                        if(x+1 < row && iceberg[x+1][y] != 0 && !visited[x+1][y]) //하
                        {
                            visited[x+1][y] = true;
                            qx[tail] = x+1; qy[tail] = y; tail++;
                        }
                        if(y-1 >= 0 && iceberg[x][y-1] != 0 && !visited[x][y-1]) //좌
                        {
                            visited[x][y-1] = true;
                            qx[tail] = x; qy[tail] = y-1; tail++;
                        }
                        if(y+1 < col && iceberg[x][y+1] != 0 && !visited[x][y+1]) //우
                        {
                            visited[x][y+1] = true;
                            qx[tail] = x; qy[tail] = y+1; tail++;
                        }
        ...
        if(label_count >= 2) //빙산이 2개 이상으로 분리된 경우
        {
            break;
        }
    }
